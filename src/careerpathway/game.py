...
import sys
import os
sys.path.append('/scratch/keummin/chaewon/src')
...
def filter_questions_by_key(questions_data, key_filter=None):
    """key 필터로 질문들을 필터링하는 함수"""
    if not key_filter:
        return questions_data
    
    filtered_data = {}
    for question_type, questions in questions_data.items():
        if question_type == "reddit-com" and key_filter:
            # reddit-com만 key로 필터링
            filtered_questions = [q for q in questions if q.get("key") == key_filter]
            filtered_data[question_type] = filtered_questions
            print(f"🔍 Key '{key_filter}'로 필터링된 {question_type} 질문: {len(filtered_questions)}개")
        else:
            filtered_data[question_type] = questions
...
def unified_balance_game(
    llm_eval, 
    output_path, 
    question_types=["word", "factor", "reddit-positive", "reddit-negative", "reddit-base"],
    ...
    questions_data = load_generated_questions("/scratch/keummin/chaewon/data/question/generated_balance_questions.jsonl")
    
    # key 필터 적용
    if key_filter:
        questions_data = filter_questions_by_key(questions_data, key_filter)
            ...
            for q in questions:
                for keyword in keywords:
                    for run in range(n_repeat):
                        ...
                        additional_fields = ["reddit_type", "extent1", "extent2", "key"]
...
def main():    
    ...
    parser.add_argument('--output', type=str, 
                       default="/scratch/keummin/chaewon/data/7b_com.jsonl")
    parser.add_argument('--reversed', action='store_true', 
                       help='Enable reversed mode (swap option1 and option2)')
    parser.add_argument('--key', type=str, default=None,
                       help='Filter reddit-com questions by key (e.g., hobby, new)')
